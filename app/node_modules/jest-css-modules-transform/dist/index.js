"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const path_1 = require("path");
const child_process_1 = require("child_process");
const postcss_1 = __importDefault(require("postcss"));
const parser_1 = __importDefault(require("./parser"));
const postcss_nested_1 = __importDefault(require("postcss-nested"));
const utils_1 = require("./utils");
const CONFIG_PATH = process.env.JEST_CSS_MODULES_TRANSFORM_CONFIG || 'jest-css-modules-transform-config.js';
const postcssNested = postcss_1.default([postcss_nested_1.default]);
// eslint-disable-next-line @typescript-eslint/no-explicit-any
let postcssConfig;
let stylus;
let sass;
const moduleTemplate = `
    "use strict";

    const data = %s;

    if (typeof module === 'object' && module) {
        Object.defineProperty(data, "__esModule", {
            value: true
        });
        module.exports = new Proxy(data, {
            get(target, attr) {
                if (attr === 'default') {
                    return target;
                };
                return target[attr];
            },

            getPrototypeOf() {
                return Object;
            }
        });
    } else {
        Object.defineProperty(exports, "__esModule", {
            value: true
        });
        exports.default = data;
    }
`;
let preProcessorsConfig;
let globalSassData;
const getGlobalSassData = (rootDir) => {
    try {
        return `${require(path_1.resolve(rootDir, '.sassrc.js')).data}\n` || '';
    }
    catch (e) {
        return '';
    }
};
const getFileExtension = (path) => {
    const filename = path.slice(path.lastIndexOf(path_1.sep) + 1);
    return filename.slice(filename.lastIndexOf('.') + 1);
};
const getSassContent = (src, path, extention, rootDir) => {
    sass = sass || require('node-sass');
    globalSassData = globalSassData === undefined ? getGlobalSassData(rootDir) : globalSassData;
    const sassConfig = Object.assign(preProcessorsConfig.sassConfig || {}, {
        data: globalSassData + src,
        file: path,
        indentedSyntax: extention === 'sass',
    });
    return String(sass.renderSync(sassConfig).css);
};
let parser;
let configPath = '';
let postcssConfigPath = '';
const lessPath = path_1.resolve(__dirname, 'less.js');
const postcssPath = path_1.resolve(__dirname, 'postcss.js');
const nodeExecOptions = {
    encoding: 'utf-8',
    maxBuffer: 1024 * 1024 * 1024,
};
const moduleTransform = {
    process(src, path, config) {
        configPath = configPath || path_1.resolve(config.rootDir, CONFIG_PATH);
        preProcessorsConfig = preProcessorsConfig || utils_1.getPreProcessorsConfig(configPath);
        parser = parser || new parser_1.default(preProcessorsConfig.cssLoaderConfig);
        const extention = getFileExtension(path);
        let textCSS = '';
        let stylusConfig;
        switch (extention) {
            case 'styl':
                stylus = stylus || require('stylus');
                stylusConfig = Object.assign(preProcessorsConfig.stylusConfig || {}, { filename: path });
                stylus.render(src, stylusConfig, (err, css) => {
                    if (err) {
                        throw err;
                    }
                    textCSS = css;
                });
                break;
            case 'sass':
            case 'scss':
                textCSS = getSassContent(src, path, extention, config.rootDir);
                break;
            case 'less':
                textCSS = child_process_1.execSync(`node ${lessPath} ${path} ${configPath}`, nodeExecOptions);
                break;
            case 'css':
            case 'pcss':
            case 'postcss':
                postcssConfigPath = '' || path_1.resolve(config.rootDir, 'postcss.config.js');
                postcssConfig = postcssConfig || preProcessorsConfig.postcssConfig || utils_1.requirePostcssConfig(postcssConfigPath);
                if (postcssConfig) {
                    textCSS = child_process_1.execSync(`node ${postcssPath} ${path} ${configPath} ${postcssConfigPath}`, nodeExecOptions);
                }
                else {
                    textCSS = postcssNested.process(src);
                }
                break;
        }
        return moduleTemplate.replace('%s', JSON.stringify(parser.getCSSSelectors(textCSS)));
    },
};
module.exports = moduleTransform;
